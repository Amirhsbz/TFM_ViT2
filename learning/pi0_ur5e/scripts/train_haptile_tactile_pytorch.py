"""
PyTorch training entrypoint for HaptileTactilePI0Pytorch (PI0Pytorch + FTP1-style tactile
expert), with multi-GPU and multi-node (DDP) support.

Fork of $OPENPI_ROOT/scripts/train_pytorch.py -- identical in every respect (DDP setup, data
loading via openpi.training.data_loader, optimizer/LR schedule, checkpointing, wandb logging)
except the model-construction block, which builds a HaptileTactilePI0Pytorch from a
HaptileTactileConfig instead of a plain PI0Pytorch from a Pi0Config. Kept as a separate script
rather than editing train_pytorch.py in place, to avoid any risk to the existing training path.

Usage: same as train_pytorch.py, but pass the "pi0_ur5e_cup_tactile" config name (or another
TrainConfig whose `model` is a HaptileTactileConfig):
  python scripts/train_haptile_tactile_pytorch.py pi0_ur5e_cup_tactile --exp_name <run_name>
"""
...
import openpi.models_pytorch.haptile_tactile_config
import openpi.models_pytorch.haptile_tactile_pytorch
...
def train_loop(config: _config.TrainConfig):
    ...
    # Build model -- the only block that differs from train_pytorch.py: this script only ever
    # trains a HaptileTactileConfig/HaptileTactilePI0Pytorch (no Pi0Config fallback-conversion
    # path, since that model has no tactile branch at all).
    if not isinstance(config.model, openpi.models_pytorch.haptile_tactile_config.HaptileTactileConfig):
        raise TypeError(
            f"train_haptile_tactile_pytorch.py requires a HaptileTactileConfig model, got "
            f"{type(config.model).__name__}. Use the pi0_ur5e_cup_tactile TrainConfig (or "
            "another one whose `model` is a HaptileTactileConfig), not pi0_ur5e_cup -- that one "
            "trains plain PI0Pytorch via train_pytorch.py instead."
        )
