...
def main(args):
    ...
    # Audio capture from Logitech mic (data collection only — saved as audio.wav per trajectory)
    ...
    if args.use_audio:
        try:
            ...
            audio_stream = sd.RawInputStream(
                device=args.audio_device if args.audio_device >= 0 else None,
                channels=args.audio_channels,
                samplerate=args.audio_sample_rate,
                blocksize=args.audio_chunk_size,
                dtype="int16",
                callback=_audio_callback,
            )
            audio_stream.start()
            print(f"Audio capture started: device={args.audio_device}, rate={args.audio_sample_rate} Hz")
        except Exception as exc:
            print(f"Audio capture disabled: {exc}")
            audio_stream = None
        ...
        while True:  # outer loop: one iteration per trajectory
            ...
            if args.save_data:
                ...
                if audio_stream is not None:
                    ...
                    wav_writer.setframerate(args.audio_sample_rate)
                    print(f"Audio file: {save_path / 'audio.wav'}")
            ...
            try:
                while True:
                    ...
                    if wav_writer is not None:
                        while True:
                            try:
                                wav_writer.writeframes(audio_queue.get_nowait())
                            except queue.Empty:
                                break
