...
#SBATCH --job-name=convert_pi0_haptile_tactile_turn_cleanser_water_bottle
...
DATASET_NAME=turn_cleanser_water_bottle             # e.g. fold_Tshirt
OUTPUT_NAME=${DATASET_NAME}_lerobot_tactile_raw
REPO_ID=local/pi0_ur5e_${DATASET_NAME}_tactile_raw
DEFAULT_PROMPT="Pick up the cleanser bottle, tilt it over either the yellow bowl or the blue bowl as if pouring, then place it back in its original position"          # e.g. "Fold the t-shirt in half"
