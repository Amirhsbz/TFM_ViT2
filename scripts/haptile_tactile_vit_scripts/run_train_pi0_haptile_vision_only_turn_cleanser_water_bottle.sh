...
#SBATCH --job-name=train_pi0_haptile_vision_only_turn_cleanser_water_bottle
#SBATCH --gres=gpu:1
...
DATASET_NAME=turn_cleanser_water_bottle                # must match run_convert_pi0_lerobot_tactile_raw.sh's DATASET_NAME
DEFAULT_PROMPT="Pick up the cleanser bottle, tilt it over either the yellow bowl or the blue bowl as if pouring, then place it back in its original position"            # must match run_convert_pi0_lerobot_tactile_raw.sh's DEFAULT_PROMPT
